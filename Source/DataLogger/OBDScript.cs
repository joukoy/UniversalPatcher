...
namespace UniversalPatcher
{
    public class OBDScript
    {
        ...
        public bool ReConnect(string line)
        {
            ...
            else
            {
                ...
                {
                    frmlogger.ConnectJConsole2(initParameters2);
                }
            }
            return true;
        }
        ...
        public bool HandleLine(string Line, string Line2, ref bool breakloop)
        {
                ...
                if (Line.Length > 1)
                {
                        ...
                        if (setParts.Length > 2)
                        {
                            switch (setParts[1].ToLower())
                            {
                                ...
                                case "responsematch":
                                    ...
                                    {
                                        responsematch = true;
                                        Debug.WriteLine("Setting responsematch on");
                                    }
                                    else
                                    {
                                        responsematch = false;
                                        Debug.WriteLine("Setting responsematch off");
                                    }
                                    if (setParts.Length > 4)
                                    {
                                        int.TryParse(setParts[3], out responsematchTimeout);
                                        int.TryParse(setParts[4], out responsematchTotalTimeout);
                                    }
                                    break;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        for (int r = 0; r < responses;)
                        {
                            ...
                            if (responsematch)
                            {
                                Debug.WriteLine("Waiting for response match");
                                device.SetReadTimeout(responsematchTimeout);
                                for(; ; )
                                {
                                    if (DateTime.Now.Subtract(starttime) > TimeSpan.FromMilliseconds(responsematchTotalTimeout))
                                    {
                                        Debug.WriteLine("Timeout waiting for response match");
                                        break;
                                    }
                                    if (rMsg != null)
                                    {
                                        if (rMsg[3] == (oMsg[3] + 0x40) || rMsg[3] == 0x7f)
                                        {
                                            Debug.WriteLine("Response match found: " + oMsg.ToString());
                                            break;
                                        }
                                        else
                                        {
                                            Debug.WriteLine("Response don't match: " + oMsg.ToString());
                                        }
                                    }
                                    if (!useSecondaryProtocol)
                                        rMsg = device.ReceiveMessage(true);
            ...
            catch (Exception ex)
            {
                ...
                Debug.WriteLine("Error, OBDScript line " + line + ": " + ex.Message);
                return false;
            }
        }
        ...
        public void UploadScript(string FileName)
        {
            try
            {
                ...
                {
                    if (Line.StartsWith("connect") && row == 0)
                    {
                        row++;
                        continue; //Already connected, skip connect in fist line
                    }
                    if (!Line.StartsWith("#"))
                    {
                        scriptrows.Add(Line);
