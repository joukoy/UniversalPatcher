...
namespace PcmHacking
{
    ...
    public class OBDXProDevice : SerialDevice
    {
        ...
        public Response<Message> FindResponseFromTool(byte[] expected)
        {
            //this.Logger.AddDebugMessage("FindResponse called");
            for (int iterations = 0; iterations < 5; iterations++)
            {
                Response<Message> response = this.ReadDVIPacket(this.GetReceiveTimeout());
        ...
        /// </summary>
        private Response<Message> ReadDVIPacket(int timeout = 0)
        {
            UInt16 Length = 0;
        ...
        /// </summary>
        private Response<Message> SendDVIPacket(Message message)
        {
        ...
        private Response<string> GetBoardDetails()
        {
            ...
            Response<Message> m = ReadDVIPacket();
            if (m.Status == ResponseStatus.Success)
            {
                byte[] Val = m.Value.GetBytes();
        ...
        private bool SetProtocol(OBDProtocols val)
        {
            ...
            Response<Message> m = FindResponseFromTool(RespBytes);
            if (m.Status == ResponseStatus.Success)
            {
        ...
        private bool SetToFilter(byte Val)
        {
            ...
            RespBytes[0] += (byte)0x10;
            RespBytes[RespBytes.Length - 1] = CalcChecksum(RespBytes);
            Response<Message> response = ReadDVIPacket();
