...
namespace PcmHacking
{
    ...
    public partial class Vehicle : IDisposable
    {
        ...
        /// </summary>
        public DpidCollection ConfigureDpids(DpidConfiguration dpidConfiguration, uint osid)
        {
                ...
                foreach (ParameterGroup group in dpidConfiguration.ParameterGroups)
                {
                    ...
                    foreach (LogColumn column in group.LogColumns)
                    {
                        ...
                        if (pidParameter != null)
                        {
                            ...
                            if (!this.SendMessage(configurationMessage))
                            {
                                return null;
                            }
            ...
            catch (Exception exception)
            {
                ...
                Debug.WriteLine("Error, ConfigureDpids line " + line + ": " + exception.Message, "Error");
            }
        ...
        /// </summary>
        public bool RequestDpids(DpidCollection dpids)
        {
            try
            {
                Message startMessage = this.protocol.RequestDpids(dpids, this.UsePassiveMode);
                if (!this.SendMessage(startMessage))
                {
                    return false;
                }
        ...
        /// <returns></returns>
        public Response<int> GetPid(UInt32 pid)
        {
            Message request = this.protocol.CreatePidRequest(pid);
            if(!this.TrySendMessage(request, "PID request"))
            {
