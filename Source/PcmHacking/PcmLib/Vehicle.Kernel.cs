...
namespace PcmHacking
{
    ...
    public partial class Vehicle : IDisposable
    {
        ...
        public void SuppressChatter()
        {
            ...
            this.device.SendMessage(suppressChatter);
            this.notifier.ForceNotify();

            Stopwatch stopwatch = new Stopwatch();
            stopwatch.Start();
            while (stopwatch.ElapsedMilliseconds < 1000)
            {
                Message received = this.device.ReceiveMessage();
                if (received != null)
                {
        ...
        /// </summary>
        private Response<bool> WriteBlock(byte block, byte[] data)
        {
            Message m;
        ...
        /// </summary>
        public Response<byte[]> LoadKernelFromFile(string path)
        {
            ...
            try
            {
                using (Stream fileStream = File.OpenRead(path))
                {
                    ...
                    int bytesRead = fileStream.Read(file, 0, (int)fileStream.Length);
        ...
        public void ExitKernel()
        {
            ...
            if (device.Supports4X)
            {
                ...
                this.device.SendMessage(exitKernel);
        ...
        public void ClearTroubleCodes()
        {
            ...
            this.device.SetTimeout(TimeoutScenario.Minimum);
            ...
            Thread.Sleep(250);
            this.device.SendMessage(clearCodesRequest);
            Thread.Sleep(250);
            this.device.SendMessage(clearCodesRequest);
            ...
            Thread.Sleep(250);
            this.device.SendMessage(clearDiagnosticInformationRequest);
            Thread.Sleep(250);
            this.device.SendMessage(clearDiagnosticInformationRequest);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Response<UInt32> QueryOperatingSystemIdFromKernel(CancellationToken cancellationToken)
        {
            this.device.SetTimeout(TimeoutScenario.ReadProperty);
        ...
        /// </summary>
        public UInt32 QueryFlashChipId(CancellationToken cancellationToken)
        {
            for (int retries = 0; retries < 3; retries++)
            {
                this.SetDeviceTimeout(TimeoutScenario.ReadProperty);
                Query<UInt32> chipIdQuery = this.CreateQuery<UInt32>(
                    ...
                    cancellationToken);
                Response<UInt32> chipIdResponse = chipIdQuery.Execute();
        ...
        /// </summary>
        /// <returns></returns>
        public UInt32 GetKernelVersion()
        {
            Message query = this.protocol.CreateKernelVersionQuery();
            for (int retryCount = 0; retryCount < 5; retryCount++)
            {
                if (!this.device.SendMessage(query))
                {
                    Thread.Sleep(100);
                    continue;
                }

                Message reply = this.device.ReceiveMessage();
                if (reply == null)
                {
                    Thread.Sleep(100);
                    continue;
        ...
        /// </summary>
        public bool PCMExecute(byte[] payload, int address, CancellationToken cancellationToken)
        {
            ...
            for (int attempt = 0; attempt < 5; attempt++)
            {
                ...
                if (!TrySendMessage(request, "upload request"))
                {
                    return false;
                }

                if (this.WaitForSuccess(this.protocol.ParseUploadPermissionResponse, cancellationToken))
                {
            ...
            this.device.SetTimeout(TimeoutScenario.SendKernel);
            ...
            notifier.Notify();
            Response<bool> uploadResponse = WritePayload(remainderMessage, cancellationToken);
            ...
            for (int chunkIndex = chunkCount; chunkIndex > 0; chunkIndex--)
            {
                ...
                uploadResponse = WritePayload(payloadMessage, cancellationToken);
                if (uploadResponse.Status != ResponseStatus.Success)
                {
            ...
            if (ReportKernelID)
            {
                ...
                UInt32 kernelVersion = this.GetKernelVersion();
        ...
        /// </summary>
        public bool VehicleSetVPW4x(VpwSpeed newSpeed)
        {
            if (!device.Supports4X) 
            {
            ...
            if (newSpeed == VpwSpeed.FourX)
            {
                ...
                device.SetTimeout(TimeoutScenario.ReadProperty);
                ...
                List<byte> modules = this.RequestHighSpeedPermission(notifier);
                if (modules == null)
                {
                ...
                while (((response = this.device.ReceiveMessage()) != null) && (sw.ElapsedMilliseconds < 500))
                {
        ...
        /// </summary>
        private List<byte> RequestHighSpeedPermission(ToolPresentNotifier notifier)
        {
            Message permissionCheck = this.protocol.CreateHighSpeedPermissionRequest(DeviceId.Broadcast);
            this.device.SendMessage(permissionCheck);
            ...
            while ((response = this.device.ReceiveMessage()) != null)
            {
        ...
        /// </summary>
        public Response<bool> WritePayload(Message message, CancellationToken cancellationToken)
        {
            int retryCount = 0;
            for (; retryCount < MaxSendAttempts; retryCount++)
            {
                ...
                if (this.WaitForSuccess(this.protocol.ParseUploadResponse, cancellationToken))
                {
                ...
                Thread.Sleep(100);
                this.SendToolPresentNotification();
            }
