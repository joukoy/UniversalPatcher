...
namespace InverseParserTK
{
    ...
    public class InverseParser
    {
        ...
        public InverseParser(char decimalSeparator)
        {
        ...
        public double InverseParse(string expression,double startValue)
        {
            bool isRadians = false;
            ...
            try
            {
                return Calculate(ConvertToRPN(FormatString(expression)),startValue);
            }
        ...
        private string LexicalAnalysisInfixNotation(string expression, ref int pos)
        {
            ...
            // If it is a operator
            else if (supportedOperators.ContainsKey(token.ToString()))
            {
...
/*            else if (token[0] == FunctionMarker[0])
        ...
        /// <returns>Result</returns>
        private double Calculate(string expression, double startValue)
        {
            int pos = parts.Count-1; // Current position of lexical analysis
            double res = startValue;    //Current value

            // Analyse entire expression
            while (pos > -1)
            {
                if (parts[pos].StartsWith(OperatorMarker))
                {
                    string token = parts[pos];
                    res = SyntaxAnalysisRPN(token,ref pos, res);                    
                }
                pos--;               
            }
        ...
        private double SyntaxAnalysisRPN(string token, ref int pos, double rst)
        {
            // Apply operator or function to elements in stack
            int pendingOperation = -1;

            if (NumberOfArguments(token) == 1)
            {
                double arg = rst;
            ...
            else
            {
                ...
                double arg2 = rst;
                double arg1 = double.MaxValue;
                pos--;
                string part = parts[pos];
                if (part.StartsWith(OperatorMarker))
                {
                    pendingOperation = pos;
                    int skipNumbers = 0; 
                    while (part.StartsWith(OperatorMarker))
                    {
                        skipNumbers += NumberOfArguments(part) - 1;
                        pos--;
                        part = parts[pos];
                    }
                    pos -= skipNumbers; //How many number parameters are "used" from list for pending operations
                    part = parts[pos];
                }
                if (part != NumberMaker + "x")
                    arg1 = double.Parse(part.Remove(0, 1));
                else
                {
                    //Handle X calculation
                    part = parts[pos - 1];
                    arg1 = double.Parse(part.Remove(0, 1));
                    if (token == Divide)
                    {
            ...
            {
                string part = parts[pendingOperation];
                rst = SyntaxAnalysisRPN(part, ref pendingOperation, rst);
            }
